...
/*		// TODO: Color of second vertex should be preserved
		...
		buf[1].texturecoords = Vec2<float,float>(data[0].texturecoords.x, data[1].texturecoords.y);

		buf[2].clippos = ClipCoords(data[1].clippos.x, data[0].clippos.y, data[1].clippos.z, data[1].clippos.w);
		buf[2].texturecoords = Vec2<float,float>(data[1].texturecoords.x, data[0].texturecoords.y);
